@extends('layouts.admin', ['title' => 'Tambah Project'])
        ...
        <form action="{{ route('projek.store') }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group">
                ...
                <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama" name="nama" placeholder="Masukkan nama projek anda.." value="{{ old('nama') }}">
                @error('nama')
                <div class="invalid-feedback">
                  {{ $message }}
                </div>
            ...
            <div class="col-md-6">
              <div class="form-group">
                <label for="jenis" class="form-label">Jenis projek</label>
                <select autocomplete="off" id="jenis" class="form-control form-select @error('jenis') is-invalid @enderror" name="jenis">
                  <option selected disabled>Pilih jenis project</option>
                  @foreach($jenis_projek as $jp)
                  <option value="{{ $jp }}" @if(old('jenis')==$jp) selected @endif>
                    {{ $jp }}
                  </option>
                  @endforeach
                </select>
          ...
          <div class="mb-3">
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  <label for="link" class="form-label">Link/Url projek</label>
                  <input type="text" class="form-control @error('link') is-invalid @enderror" name="link" id="link" value="{{ old('link') }}" placeholder="Masukkan link/url projek anda..">
                  @error('link')
                  <div class="invalid-feedback">
                    {{ $message }}
                  </div>
              ...
              <div class="col-md-6">
                <div class="form-group">
                  ...
                  <textarea class="form-control @error('deskripsi') is-invalid @enderror" rows="4" id="deskripsi" name="deskripsi" placeholder="Masukkan deskripsi projek..">{{ old('deskripsi') }}</textarea>
                  @error('deskripsi')
                  <div class="invalid-feedback">
                    {{ $message }}
                  </div>
